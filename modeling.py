...
class AlbertForRetrieverOnlyPositivePassage(AlbertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.attention_D=nn.Linear(config.proj_size, 1)
        
        self.attention_encoder=AlbertModel(config)
        self.atten_proj = nn.Linear(config.hidden_size, config.proj_size)

        self.fuse_proj=nn.Linear(2*config.proj_size, config.proj_size)
    ...
    def forward(self, query_input_ids=None, query_attention_mask=None, query_token_type_ids=None, 
                passage_input_ids=None, passage_attention_mask=None, passage_token_type_ids=None, 
                retrieval_label=None, query_rep=None, passage_rep=None, previous_step=None):
        outputs = ()
        ...
        if query_input_ids is not None:
            ...
            if revelence_passage is not None:
                query_outputs = self.attention_encoder(query_input_ids,
                                attention_mask=query_attention_mask,
                                token_type_ids=query_token_type_ids)
                query_pooled_output = query_outputs[1]
                query_pooled_output = self.dropout(query_pooled_output)
                query_rep = self.atten_proj(query_pooled_output) # batch_size,  proj_size


                

                D_weight= F.softmax(self.attention_D(query_rep).squeeze(1))

                query_rep=torch.sum(torch.mul(query_rep,D_weight.unsqueeze(1)),0).unsqueeze(0)
...
class Explorer_GAT(AlbertPreTrainedModel):
    r"""
    
    """
    def __init__(self, config):
        super(Explorer_GAT, self).__init__(config)
        ...
        self.conv1 = GATConv(config.proj_size, config.proj_size, num_heads=128)
        # On the Pubmed dataset, use heads=8 in conv2.
        self.conv2 = GATConv(config.proj_size * 128, config.proj_size, num_heads=64)
        self.fc1 = nn.Linear(config.proj_size * 64, config.proj_size)
        self.fc2 = nn.Linear(config.proj_size * 3, config.proj_size)
        self.history_turn_embeddings=nn.Embedding(20, config.proj_size)

        self.reasoning_GRU=nn.GRU(input_size=self.proj_size, hidden_size=self.proj_size, batch_first=True,num_layers=1)
    ...
    def forward(self, query_input_ids=None, query_attention_mask=None, query_token_type_ids=None, 
                passage_input_ids=None, passage_attention_mask=None, passage_token_type_ids=None, 
                retrieval_label=None, query_rep=None, passage_rep=None,is_training=None,node_embedding=None,
                g=None,label=None,is_answer=None,history_turn_index=None,history_answer_sequence=None,args=None):
...
# GNN embedding
        x = F.dropout(node_embedding, p=0.6, training=is_training)
        x = F.elu(self.conv1(g, x))
        x=torch.reshape(x, (-1,args.proj_size * 128))
        x = F.dropout(x, p=0.6, training=is_training)
        x = self.conv2(g, x)
        x=torch.reshape(x, (-1,args.proj_size * 64))

        x=self.fc1(x)

# history_turn embedding
        
        history_turn_embedding=self.history_turn_embeddings(history_turn_index)


# +semantic and history_turn embedding
        x1= torch.cat((x, node_embedding, history_turn_embedding), 1)
        x=self.fc2(x1)

# #reasoning layer
        # history_answer_sequence_emdding=x[history_answer_sequence]   #node_num*n*proj_size




        # output, hidden=self.reasoning_GRU(history_answer_sequence_emdding)

        # x=torch.mean(hidden,0)

        # x=torch.reshape(x,(-1,self.proj_size))

        logits=torch.mm(x,query_rep.permute(1,0)).squeeze()
        logits=torch.unsqueeze(logits,0)   

        explorer_loss=CrossEntropyLoss()(logits,label.float().squeeze(-1).argmax().view(-1))


        return logits.squeeze(),explorer_loss  
...
# class Explorer_GAT(BertPreTrainedModel):
...
#         self.conv1 = GATConv(config.proj_size, 8, num_heads=8)
#         # On the Pubmed dataset, use heads=8 in conv2.
#         self.conv2 = GATConv(8 * 8, config.proj_size, num_heads=1)
#         self.fc1 = nn.Linear(config.proj_size, config.proj_size)
#         self.fc2 = nn.Linear(768, config.proj_size)
...
#                 passage_input_ids=None, passage_attention_mask=None, passage_token_type_ids=None, 
#                 retrieval_label=None, query_rep=None, passage_rep=None,is_training=None,node_embedding=None,
#                 g=None,label=None,is_answer=None):
...
#         x = F.dropout(x, p=0.6, training=is_training)
#         x = self.conv2(g, x)
#         x=torch.reshape(x, (-1,args.proj_size))



#         logits=F.log_softmax(torch.mm(self.fc1(x),query_rep.permute(1,0)).squeeze())
#         logits=torch.unsqueeze(logits,0)   

#         explorer_loss=torch.nn.NLLLoss()(logits,label.float().squeeze(-1).argmax().view(-1))
